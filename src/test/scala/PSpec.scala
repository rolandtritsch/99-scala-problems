...
import org.scalatest.{FlatSpec, BeforeAndAfter, Tag}
import org.scalatest.matchers.ShouldMatchers

class PSpec extends FlatSpec with BeforeAndAfter with ShouldMatchers {
  before {
  }
  ...
  // P8.compress2(List('a, 'a, 'a, 'a, 'b, 'c, 'c, 'a, 'a, 'd, 'e, 'e, 'e, 'e)) should equal (List('a, 'b, 'c, 'a, 'd, 'e))
    P8.compress2(List('a, 'a, 'a, 'a, 'b, 'c, 'c, 'a, 'a, 'd, 'e, 'e, 'e, 'e)) should equal (List('a, 'b, 'c, 'd, 'e))
  }
...
*/
  ...
  "P12-extended" should "decode a run-length encoded list" taggedAs(Tag("ExtendedTest")) ignore {
    val encoded = List((4, 'a), (1, 'b), (2, 'c), (2, 'a), (1, 'd), (4, 'e))
    val decoded = List('a, 'a, 'a, 'a, 'b, 'c, 'c, 'a, 'a, 'd, 'e, 'e, 'e, 'e)
  ...
  "P16" should "drop every Nth element from a list" in {
    P16.drop(3, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)) should equal (
  ...
  "P20-extended" should "remove the Kth element from a list" taggedAs(Tag("Extended")) in {
    val input = List('a, 'b, 'c, 'd)
    val result = (List('a, 'c, 'd),'b)
  ...
  "P23-extended" should "extract a given number of randomly selected elements from a list" taggedAs(Tag("ExtendedTest")) in {
    P23.randomSelect2(3, List('a, 'b, 'c, 'd, 'f, 'g, 'h)) should have size (3)
  }
  ...
  "P27" should "group the elements of a set into disjoint subsets" ignore {
    P27.group3(List("Aldo", "Beat", "Carla", "David", "Evi", "Flip", "Gary", "Hugo", "Ida")) should equal (
      List(List(List("Aldo", "Beat"), List("Carla", "David"), List("Evi", "Flip", "Gary", "Hugo", "Ida")))
    )

    P27.group(List(2, 2, 5), List("Aldo", "Beat", "Carla", "David", "Evi", "Flip", "Gary", "Hugo", "Ida")) should equal (
      List(List(List("Aldo", "Beat"), List("Carla", "David"), List("Evi", "Flip", "Gary", "Hugo", "Ida")))
    )
  }

  "P28" should "sort a list of lists according to length of sublists" ignore {
    P28.lsort(List(List('a, 'b, 'c), List('d, 'e), List('f, 'g, 'h), List('d, 'e), List('i, 'j, 'k, 'l), List('m, 'n), List('o))) should equal (
      List(List('o), List('d, 'e), List('d, 'e), List('m, 'n), List('a, 'b, 'c), List('f, 'g, 'h))
    )

    P28.lsortFreq(List(List('a, 'b, 'c), List('d, 'e), List('f, 'g, 'h), List('d, 'e), List('i, 'j, 'k, 'l), List('m, 'n))) should equal (
      List(List('i, 'j, 'k, 'l), List('o), List('a, 'b, 'c), List('f, 'g, 'h), List('d, 'e))
    )
  }
